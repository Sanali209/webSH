# Design Document: PC Center OS (v2.0)
...
## 5. Система Плагинов (Plugin System)
...
### 5.3. Unified Settings (Настройки)
...
1.  **Permission Scoping:** Плагин должен запросить права (`network`, `filesystem.write`) в манифесте. Ядро может запретить загрузку плагина, если права чрезмерны.
2.  **Isolation:** Ошибки в одном плагине изолируются Middleware и не роняют сервер.
3.  **Validation:** Все данные на входе (API) и выходе (Event Bus) валидируются Pydantic-схемами.
